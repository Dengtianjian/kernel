...
# Provisioner
## Install
在Provisioner/Install文件夹下面。  
### Install类
```php
<?php

//* 变更为当前插件的ID
namespace kernel\Provisioner\Install;
...
//* 替换插件ID
$Iuu = new Provisioner("kernel", $_GET['fromversion']);
...
```
### 目录结构
当需要编写版本更新处理时，需要在`Provisioner/Upgrade`文件夹下创建一个文件夹，文件夹名称使用**数字，以及 . 分隔开来**，例如：`0.2.1`、`1.15.6`，然后在里面创建一个以`Upgrade_`为前缀，再加上以版本号用`_`分隔的文件名称，例如：`Upgrade_0_2_1`，`Upgrade_1_15_1`，该文件名同文件内的类名称一致。
- Provisioner
  - Upgrade
    - 0.2.1
      - Upgrade_0_2_1.php
    - 1.15.1
      - Upgrade_1_15_1.php
### 多编码SQL
假设是0.2.1版本，就在`Provisioner/Upgrade`下创建一个0.2.1文件夹，里面放置`Upgrade_0_2_1.php`文件夹，同时存放需要支持的**大写形式以编码名称为文件名称**的SQL文件，例如`GBK.sql`、`UTF-8.sql`，然后在`Upgrade_0_2_1`里读取不同编码的文件内容，通过`runquery`执行。  
文件夹结构
- Provisioner
  - Upgrade
    - 0.2.1
...
// 替换插件ID
$Iuu = new Provisioner("kernel", null);
$Iuu->uninstall();
