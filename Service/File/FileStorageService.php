...
class FileStorageService extends FileService
{
   ...
   */
  static function verifyAccessAuth($FileKey, $Signature, $SignatureKey, $RawURLParams = [], $RawHeaders = [], $AuthId = null, $HTTPMethod = "get", $ACL = FileStorage::PRIVATE)
  {
    if (!array_key_exists("signature", $RawURLParams)) {
      $RawURLParams['signature'] = $Signature;
    }
    if (!array_key_exists("acl", $RawURLParams)) {
      $RawURLParams['acl'] = $ACL;
    }

    return FileStorage::verifyAccessAuth($SignatureKey, $FileKey, $RawURLParams, $RawHeaders, $AuthId, $HTTPMethod);
  }
  /**
   * 获取访问授权信息字符串
   *
   ...
   */
  static function getAccessAuth($FileKey, $SignatureKey, $Expires = 600, $URLParams = [], $AuthId = null, $HTTPMethod = "get", $ACL = FileStorage::PRIVATE, $ToString = false)
  {
    ...
    $AccessAuth = FileStorage::generateAccessAuth($FileKey, $SignatureKey, $Expires, $URLParams, $AuthId, $HTTPMethod, $ACL, $ToString);
